...
const Active = () => {
  ...
  const handleAddTodo = (e) => {
    ...
    if (editingTodo) {
      axios
        .put(`http://localhost:3002/todos/${editingTodo.id}`, {
  ...
  const handleCheck = (id, isChecked) => {
    axios
      .put(`http://localhost:3002/todos/${id}`, {
    ...
    <div className="container mt-5">
      ...
      <div className="d-flex flex-wrap">
        {todoList.map(({ id, todo, isCompleted }) => (
          <Card key={id} className={`m-2 ${isCompleted ? "completed-card" : "initial-card"}`} style={{ width: "18rem" }}>
            <Card.Body>
              <Card.Title
